...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, string> = {
    ...
    false: 'bg-white/5 text-white/30 border-white/10',
  };
...
function ServiceStatusBadge({ status }: { status: 'healthy' | 'degraded' | 'down' | 'checking' }) {
  const map = {
    ...
    checking: 'bg-white/5 text-white/30 border-white/10',
  };
  return (
...
function DiagnosticsTab() {
  ...
  return (
    <div className="space-y-6">
      {/* Header Card */}
      <div className="bg-black/40 border border-white/08 rounded-2xl p-6 mb-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h2 className="font-display text-xl font-700 text-white mb-1">System Diagnostics</h2>
          ...
          <button
            ...
            disabled={loading}
            className="flex items-center gap-2 bg-gold-500/15 hover:bg-gold-500/25 border border-gold-500/30 text-gold-400 rounded-xl px-5 py-2.5 text-sm font-bold transition-all"
          >
            {loading ? (
              <>
        ...
        {result && (
          <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="bg-white/03 border border-white/08 rounded-xl p-4 text-center">
              <p className="text-white/40 text-xs font-bold tracking-widest uppercase mb-1">Health Score</p>
              <p className={`font-display text-3xl font-700 ${scorePercent >= 90 ? 'text-green-400' : scorePercent >= 70 ? 'text-yellow-400' : 'text-red-400'}`}>
                {scorePercent}%
              </p>
            </div>
            <div className="bg-green-500/05 border border-green-500/15 rounded-xl p-4 text-center">
              <p className="text-white/40 text-xs font-bold tracking-widest uppercase mb-1">Passed</p>
              <p className="font-display text-3xl font-700 text-green-400">{result.passed}</p>
            </div>
            <div className="bg-red-500/05 border border-red-500/15 rounded-xl p-4 text-center">
              <p className="text-white/40 text-xs font-bold tracking-widest uppercase mb-1">Failed</p>
              <p className="font-display text-3xl font-700 text-red-400">{result.failed}</p>
            </div>
            <div className="bg-yellow-500/05 border border-yellow-500/15 rounded-xl p-4 text-center">
              <p className="text-white/40 text-xs font-bold tracking-widest uppercase mb-1">Warnings</p>
              <p className="font-display text-3xl font-700 text-yellow-400">{result.warnings}</p>
            </div>
          </div>
        )}
      </div>
      ...
      {result && (
        <>
            ...
            {(['all', 'fail', 'warn', 'pass'] as const).map((f) => (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-bold tracking-wide transition-all border ${
                  filter === f
                    ? (f === 'fail' ? 'bg-red-500/15 border-red-500/30 text-red-400'
                      : f === 'warn' ? 'bg-yellow-500/15 border-yellow-500/30 text-yellow-400'
                      : f === 'pass' ? 'bg-green-500/15 border-green-500/30 text-green-400' : 'bg-gold-500/15 border-gold-500/30 text-gold-400')
                    : 'bg-white/03 border-white/08 text-white/40 hover:text-white/60'
                }`}
              >
          ...
          {categories.map((cat) => {
            ...
            return (
              <div key={cat} className="bg-black/40 border border-white/08 rounded-2xl overflow-hidden">
                <div className="px-6 py-4 border-b border-white/08 flex items-center gap-3">
                ...
                <div className="divide-y divide-white/05">
                  {catChecks.map((check) => (
                    <div key={check.id} className={`px-6 py-4 border-l-2 ${check.status === 'fail' ? 'border-l-red-500/50' : check.status === 'warn' ? 'border-l-yellow-500/50' : check.status === 'pass' ? 'border-l-green-500/50' : 'border-l-white/10'}`}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {check.durationMs !== undefined && (
                              <span className="text-white/20 text-xs">{check.durationMs}ms</span>
                            )}
                          </div>
                          <p className="text-white/50 text-sm mt-0.5">{check.message}</p>
                          {check.detail && (
                            <p className="text-white/30 text-xs font-mono mt-1 bg-white/03 rounded px-2 py-1">{check.detail}</p>
                          )}
                          {check.fixSuggestion && (
                            <div className="mt-2">
                              <button
                              ...
                              {expandedFix === check.id && (
                                <div className="mt-2 bg-gold-500/05 border border-gold-500/15 rounded-lg px-3 py-2">
                                  <p className="text-gold-300/80 text-xs">{check.fixSuggestion}</p>
                                </div>
                              )}
                            </div>
                          )}
      ...
      {!result && !loading && (
        <div className="bg-black/40 border border-white/08 rounded-2xl p-16 text-center">
          <svg className="w-12 h-12 text-white/15 mx-auto mb-4" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
          </svg>
          <p className="text-white/30 text-sm mb-4">No diagnostic data yet</p>
          <button
            onClick={runDiagnostics}
            className="inline-flex items-center gap-2 bg-gold-500/15 hover:bg-gold-500/25 border border-gold-500/30 text-gold-400 rounded-xl px-5 py-2.5 text-sm font-bold transition-all"
          >
...
function AIAgentTab() {
  ...
  const formatMessage = (content: string) => {
    ...
    return parts.map((part, i) => {
      if (part.startsWith('```') && part.endsWith('```')) {
        ...
        return (
          <pre key={i} className="bg-navy-950/80 border border-white/10 rounded-lg p-3 my-2 overflow-x-auto text-xs font-mono text-green-300/80 whitespace-pre-wrap">
            {code}
          </pre>
        );
      }
      if (part.startsWith('`') && part.endsWith('`')) {
        return (
          <code key={i} className="bg-white/10 text-gold-300 px-1.5 py-0.5 rounded text-xs font-mono">
            {part.slice(1, -1)}
          </code>
        );
      }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-200px)] min-h-[600px]">
      {/* Header */}
      <div className="bg-black/40 border border-white/08 rounded-2xl p-5 mb-4 flex items-center gap-4">
        ...
        <div>
          <h2 className="font-display text-lg font-700 text-white">CourtCraft AI Diagnostic Agent</h2>
          <p className="text-white/40 text-sm">Powered by Claude — Ask anything about errors, fixes, or platform issues</p>
        </div>
        {messages.length > 0 && (
          <button
            onClick={() => setMessages([])}
      ...
      {/* Messages Area */}
      <div className="flex-1 bg-black/40 border border-white/08 rounded-2xl overflow-hidden flex flex-col">
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {messages.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-center py-8">
              ...
              <p className="text-white/40 text-sm mb-6">Ask me about errors, fixes, or anything about the CourtCraft platform</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 w-full max-w-2xl">
                {quickPrompts.map((prompt, i) => (
                  <button
                    key={i}
                    onClick={() => { setInput(prompt); textareaRef.current?.focus(); }}
                    className="text-left bg-white/03 hover:bg-white/06 border border-white/08 hover:border-white/15 rounded-xl px-4 py-3 text-xs text-white/50 hover:text-white/70 transition-all"
                  >
                    {prompt}
                  </button>
                ))}
          ...
          {messages.map((msg, i) => (
            <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              ...
              <div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${
                msg.role === 'user' ?'bg-gold-500/15 border border-gold-500/20 text-white/90 rounded-tr-sm' :'bg-white/05 border border-white/08 text-white/80 rounded-tl-sm'
              }`}>
              ...
              {msg.role === 'user' && (
                <div className="w-7 h-7 rounded-lg bg-white/08 border border-white/10 flex items-center justify-center flex-shrink-0 mt-1">
          ...
          {loading && (
            <div className="flex gap-3 justify-start">
              <div className="w-7 h-7 rounded-lg bg-gold-500/15 border border-gold-500/20 flex items-center justify-center flex-shrink-0 mt-1">
                <svg className="w-3.5 h-3.5 text-gold-400 animate-spin" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                </svg>
              </div>
              <div className="bg-white/05 border border-white/08 rounded-2xl rounded-tl-sm px-4 py-3">
        ...
        <div className="border-t border-white/08 p-4">
          <div className="flex gap-3 items-end">
            <textarea
              ...
              disabled={loading}
              rows={2}
              className="flex-1 bg-white/05 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-white/25 resize-none focus:outline-none focus:border-gold-500/40 transition-colors disabled:opacity-50"
            />
            <button
              onClick={sendMessage}
              disabled={loading || !input.trim()}
              className="flex-shrink-0 w-11 h-11 bg-gold-500/20 hover:bg-gold-500/30 border border-gold-500/30 text-gold-400 rounded-xl flex items-center justify-center transition-all disabled:opacity-40 disabled:cursor-not-allowed"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 19l9 2-9-18-9 18 9-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
              </svg>
            </button>
          </div>
        </div>
...
function HealthTab() {
  ...
  const overallColors = {
    ...
    checking: 'border-white/08 bg-white/02',
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {incidentBanner && (
        <div className="flex items-start gap-3 bg-red-500/10 border border-red-500/20 rounded-xl px-5 py-4">
          ...
          <div className="flex-1">
            <p className="text-red-400 text-sm font-bold mb-0.5">Active Incident</p>
            <p className="text-red-300/70 text-sm">{incidentBanner}</p>
          </div>
          <button onClick={() => setIncidentBanner(null)} className="text-red-400/50 hover:text-red-400 transition-colors">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>
      ...
      <div className={`rounded-2xl border p-6 ${overallColors[overallStatus as keyof typeof overallColors]}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div>
              <p className="text-white/40 text-xs font-bold tracking-widest uppercase mb-1">Overall System Status</p>
          ...
          <div className="flex items-center gap-3">
            <div className="text-right">
              <p className="text-white/30 text-xs">Retry Queue</p>
              <p className="text-white font-bold text-lg">{health?.retryQueueDepth ?? 0}</p>
            </div>
            <button
              onClick={fetchHealth}
              disabled={loading}
              className="flex items-center gap-2 bg-white/05 hover:bg-white/10 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white/60 hover:text-white transition-all"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {health ? (
          health.services.map((svc) => (
            <div key={svc.service} className="bg-black/40 border border-white/08 rounded-2xl p-5">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl bg-white/05 border border-white/08 flex items-center justify-center">
                  ...
                  <div>
                    <p className="text-white font-bold text-sm capitalize">{svc.service}</p>
                    <p className="text-white/30 text-xs">{new Date(svc.checkedAt).toLocaleTimeString('en-GB')}</p>
                  </div>
                </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-xs font-bold ${svc.latencyMs > 3000 ? 'text-red-400' : svc.latencyMs > 1000 ? 'text-yellow-400' : 'text-green-400'}`}>
                    {svc.latencyMs}ms
                  </span>
                </div>
                {svc.statusCode && (
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-xs font-bold ${svc.statusCode >= 400 ? 'text-red-400' : 'text-green-400'}`}>{svc.statusCode}</span>
                  </div>
                )}
                {svc.error && (
                  <div className="mt-3 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">
                    <p className="text-red-400 text-xs font-mono truncate">{svc.error}</p>
                  </div>
                )}
              </div>
              ...
              {svc.status !== 'healthy' && (
                <button
                  ...
                  className={`mt-4 w-full flex items-center justify-center gap-2 rounded-xl px-3 py-2 text-xs font-bold transition-all ${
                    alertStatus[svc.service] === 'sent' ?'bg-green-500/10 border border-green-500/20 text-green-400'
                      : alertStatus[svc.service] === 'failed' ?'bg-red-500/10 border border-red-500/20 text-red-400' :'bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 hover:bg-yellow-500/20'
                  }`}
                >
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
          ...
          Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="bg-black/40 border border-white/08 rounded-2xl p-5 animate-pulse">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-black/40 border border-white/08 rounded-xl p-5">
          <p className="text-white/40 text-xs font-bold tracking-widest uppercase mb-2">Avg Latency</p>
          <p className={`font-display text-2xl font-700 ${avgLatency > 2000 ? 'text-red-400' : avgLatency > 800 ? 'text-yellow-400' : 'text-green-400'}`}>
            {avgLatency}ms
          </p>
          <p className="text-white/30 text-xs mt-1">across {apiMetrics.length} calls</p>
        </div>
        <div className="bg-black/40 border border-white/08 rounded-xl p-5">
          <p className="text-white/40 text-xs font-bold tracking-widest uppercase mb-2">Success Rate</p>
          <p className={`font-display text-2xl font-700 ${successRate < 90 ? 'text-red-400' : successRate < 99 ? 'text-yellow-400' : 'text-green-400'}`}>
            {successRate}%
          </p>
          <p className="text-white/30 text-xs mt-1">{failureCount} failures</p>
        </div>
        <div className="bg-black/40 border border-white/08 rounded-xl p-5">
          <p className="text-white/40 text-xs font-bold tracking-widest uppercase mb-2">Active Incidents</p>
          <p className={`font-display text-2xl font-700 ${incidents.filter((i) => !i.resolvedAt).length > 0 ? 'text-red-400' : 'text-green-400'}`}>
            {incidents.filter((i) => !i.resolvedAt).length}
          </p>
          <p className="text-white/30 text-xs mt-1">{incidents.length} total logged</p>
        </div>
      </div>
      ...
      <div className="bg-black/40 border border-white/08 rounded-2xl overflow-hidden">
        <div className="px-6 py-5 border-b border-white/08 flex items-center justify-between">
          <h2 className="font-display text-lg font-700 text-white">API Request Monitor</h2>
          <span className="text-white/30 text-xs">Live · auto-refreshes every 60s</span>
        </div>
        {apiMetrics.length === 0 ? (
          <div className="px-6 py-8 text-center text-white/30 text-sm">No requests tracked yet. Click &quot;Check Now&quot; to begin.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/05">
                  <th className="text-left px-6 py-3 text-xs font-bold tracking-widest uppercase text-white/30">Endpoint</th>
                  <th className="text-left px-6 py-3 text-xs font-bold tracking-widest uppercase text-white/30">Method</th>
                  <th className="text-left px-6 py-3 text-xs font-bold tracking-widest uppercase text-white/30">Status</th>
                  <th className="text-left px-6 py-3 text-xs font-bold tracking-widest uppercase text-white/30">Latency</th>
                  <th className="text-left px-6 py-3 text-xs font-bold tracking-widest uppercase text-white/30">Time</th>
                </tr>
              </thead>
              <tbody>
                {apiMetrics.map((m, i) => (
                  <tr key={i} className="border-b border-white/03 hover:bg-white/02 transition-colors">
                    <td className="px-6 py-3 text-sm font-mono text-white/60">{m.endpoint}</td>
                    <td className="px-6 py-3">
                      <span className="text-xs font-bold text-blue-400 bg-blue-500/10 border border-blue-500/20 px-2 py-0.5 rounded">{m.method}</span>
                    </td>
                    <td className="px-6 py-3">
                      <span className={`text-xs font-bold px-2 py-0.5 rounded border ${m.success ? 'text-green-400 bg-green-500/10 border-green-500/20' : 'text-red-400 bg-red-500/10 border-red-500/20'}`}>
                        {m.statusCode || 'ERR'}
                      </span>
                    </td>
                    <td className="px-6 py-3">
                      <span className={`text-sm font-bold ${m.latencyMs > 3000 ? 'text-red-400' : m.latencyMs > 1000 ? 'text-yellow-400' : 'text-green-400'}`}>
                        {m.latencyMs}ms
                      </span>
                    </td>
                    <td className="px-6 py-3 text-xs text-white/30">{new Date(m.timestamp).toLocaleTimeString('en-GB')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {incidents.length > 0 && (
        <div className="bg-black/40 border border-white/08 rounded-2xl overflow-hidden">
          <div className="px-6 py-5 border-b border-white/08">
            <h2 className="font-display text-lg font-700 text-white">Incident Log</h2>
          </div>
          <div className="divide-y divide-white/05">
            {incidents.map((inc) => (
              <div key={inc.id} className="px-6 py-4 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-white text-sm font-bold capitalize">{inc.service} — {inc.type}</p>
                    <p className="text-white/30 text-xs font-mono">{inc.id}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 text-right">
                  <div>
                    <p className="text-white/30 text-xs">{new Date(inc.startedAt).toLocaleString('en-GB')}</p>
                    {inc.alertSent && (
  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <div className="bg-black border-b border-white/08 p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <AppLogo />
          <h1 className="font-display text-xl font-700 text-white">Admin Dashboard</h1>
        </div>
        <div className="flex items-center gap-3">
          <a
            href="/admin/creative-studio"
            className="flex items-center gap-2 px-4 py-2 text-sm font-bold bg-gold-500/15 hover:bg-gold-500/25 border border-gold-500/30 text-gold-400 rounded-xl transition-all"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6 0h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            Creative Studio
          </a>
          <button
      ...
      {/* Tabs */}
      <div className="bg-black border-b border-white/08 p-2">
            ...
            {tabs.map((t) => (
              <button
                ...
                className={`px-3 py-2 text-sm font-bold rounded-lg transition-all ${
                  tab === t.id
                    ? 'bg-white/10 text-white/80 border border-white/10' :'text-white/60 hover:text-white/80 hover:bg-white/5'
                }`}
              >
        ...
        <div className="max-w-7xl mx-auto p-6">
          ...
          {tab === 'overview' && (
            <div className="space-y-6">
              ...
              {/* Recent Users */}
              <div className="bg-white/05 border border-white/08 rounded-2xl p-5">
                <h3 className="font-display text-lg font-700 text-white mb-4">Recent Users</h3>
                <div className="space-y-3">
                  {data?.recentUsers.map((u) => (
                    <div key={u.id} className="flex items-center justify-between p-3 bg-white/03 rounded-xl">
                      <div className="flex items-center gap-3">
                      ...
                      <div className="text-right">
                        <p className="text-white/30 text-xs">{new Date(u.created_at).toLocaleDateString('en-GB')}</p>
              ...
              <div className="bg-white/05 border border-white/08 rounded-2xl p-5">
                <h3 className="font-display text-lg font-700 text-white mb-4">Recent Subscriptions</h3>
                <div className="space-y-3">
                  {data?.recentSubscriptions.map((s) => (
                    <div key={s.id} className="flex items-center justify-between p-3 bg-white/03 rounded-xl">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center">
                          <svg className="w-4 h-4 text-white/50" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0z" />
                        ...
                        <div>
                          <p className="text-white font-bold text-sm">{s.user_id}</p>
                          <p className="text-white/30 text-xs">{s.plan}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-white/30 text-xs">{new Date(s.created_at).toLocaleDateString('en-GB')}</p>
          ...
          {tab === 'users' && (
            <div className="space-y-6">
              {/* Filters */}
              <div className="flex items-center justify-between">
                <h3 className="font-display text-lg font-700 text-white">User Management</h3>
                <div className="flex items-center gap-2">
                  <select className="px-3 py-2 text-sm bg-white/05 border border-white/08 rounded-xl text-white/70">
                    <option>All</option>
                  ...
                  <button className="flex items-center gap-2 px-4 py-2 text-sm font-bold bg-gold-500/15 hover:bg-gold-500/25 border border-gold-500/30 text-gold-400 rounded-xl transition-all">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-white/08">
                        <th className="text-left px-6 py-3 text-xs font-bold tracking-widest uppercase text-white/30">User</th>
                        <th className="text-left px-6 py-3 text-xs font-bold tracking-widest uppercase text-white/30">Email</th>
                        <th className="text-left px-6 py-3 text-xs font-bold tracking-widest uppercase text-white/30">Status</th>
                        <th className="text-left px-6 py-3 text-xs font-bold tracking-widest uppercase text-white/30">Created</th>
                        <th className="text-left px-6 py-3 text-xs font-bold tracking-widest uppercase text-white/30">Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {data?.recentUsers.map((u) => (
                        <tr key={u.id} className="border-b border-white/03 hover:bg-white/02 transition-colors">
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="text-white font-bold text-sm">{u.full_name}</p>
                                <p className="text-white/30 text-xs">{u.is_active ? 'Active' : 'Inactive'}</p>
                              </div>
                            </div>
                          </td>
                          <td className="px-6 py-4">
                            <p className="text-white font-bold text-sm">{u.email}</p>
                            <p className="text-white/30 text-xs">{u.email_verified ? 'Verified' : 'Unverified'}</p>
                          </td>
                          <td className="px-6 py-4">
                            <StatusBadge status={u.is_active ? 'active' : 'cancelled'} />
                          </td>
                          ...
                          <td className="px-6 py-4">
                            <button className="flex items-center gap-2 px-3 py-1.5 text-xs font-bold bg-gold-500/10 hover:bg-gold-500/20 border border-gold-500/20 text-gold-400 rounded-lg transition-all">
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                              </svg>
                              Edit
          ...
          {tab === 'subscriptions' && (
            <div className="space-y-6">
              {/* Filters */}
              <div className="flex items-center justify-between">
                <h3 className="font-display text-lg font-700 text-white">Subscription Management</h3>
                <div className="flex items-center gap-2">
                  <select className="px-3 py-2 text-sm bg-white/05 border border-white/08 rounded-xl text-white/70">
                    <option>All</option>
                    <option>Active</option>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-white/08">
                        <th className="text-left px-6 py-3 text-xs font-bold tracking-widest uppercase text-white/30">User</th>
                        <th className="text-left px-6 py-3 text-xs font-bold tracking-widest uppercase text-white/30">Plan</th>
                        <th className="text-left px-6 py-3 text-xs font-bold tracking-widest uppercase text-white/30">Status</th>
                        <th className="text-left px-6 py-3 text-xs font-bold tracking-widest uppercase text-white/30">Created</th>
                      </tr>
                    </thead>
                    <tbody>
                      {data?.recentSubscriptions.map((s) => (
                        <tr key={s.id} className="border-b border-white/03 hover:bg-white/02 transition-colors">
                          <td className="px-6 py-4 text-sm text-white/80">{s.user_id}</td>
                          <td className="px-6 py-4 text-sm text-white/60 capitalize">{s.plan || 'monthly'}</td>
                          <td className="px-6 py-4"><StatusBadge status={s.status || 'pending'} /></td>
                          <td className="px-6 py-4 text-sm text-white/30">{new Date(s.created_at).toLocaleDateString('en-GB')}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          {tab === 'activity' && (
            <div className="space-y-6">
              ...
              <div className="bg-white/05 border border-white/08 rounded-2xl p-6">
                <h2 className="font-display text-lg font-700 text-white mb-5">Platform Activity Summary</h2>
                <div className="grid grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={item.label} className="flex items-center justify-between py-3 border-b border-white/05">
