  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="space-y-5">
        <div>
          <label className="block text-gray-500 text-xs font-bold tracking-widest uppercase mb-2">Ad Description</label>
          <textarea
            value={prompt}
            ...
            rows={5}
            disabled={isLoading}
            className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm text-black placeholder-gray-400 resize-none focus:outline-none focus:border-yellow-400 transition-colors disabled:opacity-50"
          />
        </div>

        <div>
          <label className="block text-gray-500 text-xs font-bold tracking-widest uppercase mb-2">Style Preset</label>
            ...
            {adStylePresets.map((s) => (
              <button
                ...
                className={`text-left px-3 py-2 rounded-lg text-xs transition-all border ${
                  style === s
                    ? 'bg-yellow-50 border-yellow-200 text-yellow-700' :'bg-gray-50 border-gray-200 text-gray-500 hover:text-gray-700 hover:border-gray-300'
                }`}
              >
                {s}
              </button>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-gray-500 text-xs font-bold tracking-widest uppercase mb-2">Size</label>
            <select
              value={size}
              onChange={(e) => setSize(e.target.value as typeof size)}
              disabled={isLoading}
              className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-black focus:outline-none focus:border-yellow-400 transition-colors"
            >
          ...
          <div>
            <label className="block text-gray-500 text-xs font-bold tracking-widest uppercase mb-2">Quality</label>
            <select
              value={quality}
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 bg-black hover:bg-gray-800 border border-black text-white rounded-xl py-3 text-sm font-bold transition-all disabled:opacity-40 disabled:cursor-not-allowed"
        >
          {isLoading ? (
            <>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden flex flex-col">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <span className="text-gray-500 text-xs font-bold tracking-widest uppercase">Preview</span>
          {savedImage && (
            <div className="flex gap-2">
              <button
                onClick={handleSave}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-50 hover:bg-gray-100 border border-gray-200 text-gray-600 hover:text-black rounded-lg text-xs font-bold transition-all"
              >
        ...
        <div className="flex-1 flex items-center justify-center p-6 min-h-[300px]">
          {isLoading ? (
            <div className="text-center">
              <div className="w-16 h-16 rounded-2xl bg-yellow-50 border border-yellow-200 flex items-center justify-center mx-auto mb-4">
                <svg className="w-8 h-8 text-yellow-500 animate-pulse" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
              </div>
              <p className="text-gray-500 text-sm">Creating your ad image...</p>
        ...
        {image?.data?.[0]?.revised_prompt && (
          <div className="px-5 py-3 border-t border-gray-100">
            <p className="text-gray-400 text-xs"><span className="text-gray-600 font-bold">Revised prompt:</span> {image.data[0].revised_prompt}</p>
          </div>
        )}
      </div>
    </div>
...
${copyType === 'Email Subject Lines' ? 'Write 5 compelling email subject lines with open rate optimisation tips.' : ''}`;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="space-y-5">
        <div>
          <label className="block text-gray-500 text-xs font-bold tracking-widest uppercase mb-2">Product / Service</label>
          <textarea
            value={product}
        ...
        <div>
          <label className="block text-gray-500 text-xs font-bold tracking-widest uppercase mb-2">Target Audience</label>
          <input
            value={audience}
            onChange={(e) => setAudience(e.target.value)}
            placeholder="e.g. UK parents aged 30-45, small business owners..."
            disabled={isLoading}
            className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm text-black placeholder-gray-400 focus:outline-none focus:border-yellow-400 transition-colors disabled:opacity-50"
          />
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-gray-500 text-xs font-bold tracking-widest uppercase mb-2">Platform</label>
            <select
              value={platform}
          ...
          <div>
            <label className="block text-gray-500 text-xs font-bold tracking-widest uppercase mb-2">Tone</label>
            <select
              value={tone}
        ...
        <div>
          <label className="block text-gray-500 text-xs font-bold tracking-widest uppercase mb-2">Copy Type</label>
          <div className="grid grid-cols-2 gap-2">
            {['Full Ad Package', 'Headlines Only', 'Social Media Captions', 'Email Subject Lines'].map((ct) => (
              <button
                ...
                className={`px-3 py-2 rounded-lg text-xs font-bold transition-all border ${
                  copyType === ct
                    ? 'bg-yellow-50 border-yellow-200 text-yellow-700' :'bg-gray-50 border-gray-200 text-gray-500 hover:text-gray-700'
                }`}
              >
                {ct}
              </button>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden flex flex-col">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <span className="text-gray-500 text-xs font-bold tracking-widest uppercase">Generated Copy</span>
          {response && (
            <div className="flex gap-2">
              <button
                onClick={handleSave}
          ...
          {isLoading ? (
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                ...
                <p className="text-gray-400 text-sm">Writing your ad copy...</p>
              </div>
            </div>
          ) : response ? (
            <div className="text-black text-sm leading-relaxed whitespace-pre-wrap">{response}</div>
          ) : (
            <div className="flex items-center justify-center h-full text-center">
              <div>
                ...
                <p className="text-gray-400 text-sm">Your generated copy will appear here</p>
              </div>
            </div>
          )}
        </div>
...
**WORD COUNT:** [approximate count]`,
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="space-y-5">
        <div>
          <label className="block text-gray-500 text-xs font-bold tracking-widest uppercase mb-2">Ad Brief</label>
          <textarea
            value={brief}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-gray-500 text-xs font-bold tracking-widest uppercase mb-2">Duration</label>
            <select
              value={duration}
        ...
        <div>
          <label className="block text-gray-500 text-xs font-bold tracking-widest uppercase mb-2">Script Style</label>
          <div className="grid grid-cols-2 gap-2">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden flex flex-col">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <span className="text-gray-500 text-xs font-bold tracking-widest uppercase">Script Output</span>
          {response && (
            <div className="flex gap-2">
              <button
                onClick={handleSave}
          ...
          {isLoading ? (
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                ...
                <p className="text-gray-400 text-sm">Writing your script...</p>
              </div>
            </div>
          ) : response ? (
            <div className="text-black text-sm leading-relaxed whitespace-pre-wrap">{response}</div>
            ...
            <div className="flex items-center justify-center h-full text-center">
              <div>
                ...
                <p className="text-gray-400 text-sm">Your voiceover script will appear here</p>
              </div>
            </div>
          )}
        </div>
...
function AudioTTSTab({ onAssetSaved }: { onAssetSaved: (asset: GeneratedAsset) => void }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="space-y-5">
        <div>
          <label className="block text-gray-500 text-xs font-bold tracking-widest uppercase mb-2">Script / Text</label>
          <textarea
            value={text}
            ...
            className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm text-black placeholder-gray-400 resize-none focus:outline-none focus:border-yellow-400 transition-colors disabled:opacity-50"
          />
          <p className="text-gray-400 text-xs mt-1">{text.length} characters · ~{Math.ceil(text.split(' ').filter(Boolean).length / 150)} min read</p>
        </div>

        <div>
          <label className="block text-gray-500 text-xs font-bold tracking-widest uppercase mb-2">Voice</label>
          <div className="grid grid-cols-2 gap-2">
            {voices.map((v) => (
              <button
                key={v.id}
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden flex flex-col">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <span className="text-gray-500 text-xs font-bold tracking-widest uppercase">Audio Output</span>
          {audioBase64 && (
            <div className="flex gap-2">
              <button
                onClick={handleSave}
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-8 min-h-[300px]">
          {isLoading ? (
            <div className="text-center">
              ...
              <p className="text-gray-500 text-sm">Synthesising voice...</p>
            </div>
          ) : audioUrl ? (
            <div className="w-full space-y-4">
              <div className="flex items-center justify-center gap-3 mb-2">
                ...
                <div>
                  <p className="text-black font-bold text-sm">{voices.find((v) => v.id === voice)?.label} Voice</p>
                  <p className="text-gray-400 text-xs">{speed}x speed · HD quality</p>
                </div>
              </div>
              <audio
                controls
                ...
                className="w-full"
              />
              <p className="text-gray-400 text-xs text-center">Use the player above to preview, then download your MP3</p>
            </div>
          ) : (
            <div className="text-center">
              <svg className="w-12 h-12 text-gray-200 mx-auto mb-3" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
              ...
              <p className="text-gray-400 text-sm">Your audio will appear here</p>
              <p className="text-gray-300 text-xs mt-1">Supports up to 4,096 characters</p>
            </div>
          )}
        </div>
      </div>
    ...
    <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
      <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-black font-bold text-sm">Saved Assets</span>
          <span className="bg-yellow-50 text-yellow-700 text-xs font-bold px-2 py-0.5 rounded-full border border-yellow-200">{assets.length}</span>
        </div>
        <button
          onClick={onClear}
        ...
        {assets.map((asset) => (
          <div key={asset.id} className="flex items-center gap-4 px-5 py-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-black text-sm font-bold truncate">{asset.label}</p>
              <p className="text-gray-400 text-xs">{new Date(asset.createdAt).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })} · {asset.type}</p>
            </div>
            <button
              onClick={() => handleDownload(asset)}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-yellow-50 hover:bg-yellow-100 border border-yellow-200 text-yellow-700 rounded-lg text-xs font-bold transition-all flex-shrink-0"
            >
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
              </svg>
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-white border-b border-gray-200 p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <AppLogo />
          <div>
            <h1 className="font-display text-xl font-700 text-black">AI Creative Studio</h1>
            <p className="text-gray-400 text-xs">Powered by OpenAI · Image Generation · Ad Copy · Voiceover · TTS</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={() => router.push('/admin/asset-gallery')}
            className="flex items-center gap-2 px-4 py-2 bg-gray-50 hover:bg-gray-100 border border-gray-200 text-gray-600 hover:text-black rounded-xl text-sm font-bold transition-all"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
            ...
            {studioTabs.map((t) => (
              ...
              >
                ...
                <div className="text-left">
                  ...
                  <p className={`text-xs font-normal leading-tight mt-0.5 ${activeTab === t.id ? 'text-yellow-600' : 'text-gray-400'}`}>{t.desc}</p>
                </div>
              </button>
            ))}
          </div>
